...
<?foreach ($tabs as $key=>$item) {
    if ($item['id']=="tab_products" || $item['id']=="tab_quote" || $item['id']=="tab_invoice" || $item['id']=="tab_order"
        || $item['id']=="tab_lists_38" || $item['id']=="tab_lists_49" || $item['id']=="tab_lists_54" || $item['id']=="tab_lists_58") {
        //unset($tabs[$key]);
        ?> .crm-entity-section-tab[data-tab-id="<?=$item['id']?>"] {
...
?><div id="<?=htmlspecialcharsbx($containerId)?>" class="crm-entity-wrap"><?
    ...
    <?if ($USER->IsAdmin()) {
        //dump($arResult['EDITOR']['ENTITY_DATA']['ASSIGNED_BY_ID']);
    }?>

    <? //if ($arResult['EDITOR']['ENTITY_DATA']['UF_CRM_LIMITEDCART']['IS_EMPTY']):
        /*$rsGroups = CGroup::GetList ($by = "c_sort", $order = "asc", Array ("STRING_ID" => "zam_rp"));
        $group_array_zam=$rsGroups->Fetch();
        $group_id_zam=$group_array_zam['ID'];
        $arGroups = $USER->GetUserGroupArray();*/

        ?>
        <?// if ($USER->IsAdmin() || $USER->GetId()==$arResult['EDITOR']['ENTITY_DATA']['ASSIGNED_BY_ID']){  //кнопка только для ответвенного видна или админу?>
        <div class="block-card">
            <a class="ui-btn ui-btn-sm ui-btn-light-border ui-btn-round" href="/germet/card_add.php?DEAL_ID=<?=$entityInfo['ENTITY_ID']?>">
                Создать таблицу
            </a>
        </div>
    <?//}?>
